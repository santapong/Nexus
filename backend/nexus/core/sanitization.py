...
_PII_PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    ...
    (
        "db_connection",
        re.compile(r"(?i)(?:postgres|mysql|mongodb)(?:\+\w+)?://[^\s]+"),
        "[REDACTED:DB_URL]",
    ),
...
def sanitize_output(output: Any, *, task_id: str = "", agent_id: str = "") -> Any:
    ...
    if isinstance(output, dict):
        return {
            k: sanitize_output(v, task_id=task_id, agent_id=agent_id) for k, v in output.items()
        }

    if isinstance(output, list):
        return [sanitize_output(item, task_id=task_id, agent_id=agent_id) for item in output]
