...
TASKS: list[dict[str, str]] = [
    ...
    {"instruction": "Research REST API best practices and write a Python example implementing them.",
     "category": "multi_agent"},
    {"instruction": "Analyze the pros and cons of microservices and write a summary email about it.",
     "category": "multi_agent"},
    {"instruction": "Research Python testing frameworks and write a comparison guide.",
     "category": "multi_agent"},
    {"instruction": "Research database indexing strategies and draft a technical recommendation.",
     "category": "multi_agent"},

    # Category 4: Sequential dependencies (tasks 14-17)
    {"instruction": "Research Python type hints best practices, then write a style guide based on the findings.",
     "category": "sequential"},
    {"instruction": "Analyze the current trends in AI development, then draft a strategy document based on the analysis.",
     "category": "sequential"},
    {"instruction": "Research GraphQL vs REST API tradeoffs, then write a technical decision document recommending one.",
     "category": "sequential"},
    {"instruction": "Research Python logging best practices, then write an implementation guide with code examples.",
     "category": "sequential"},
...
def run_stress_test() -> StressTestReport:
    ...
    with httpx.Client(timeout=30) as client:
        for i, task_def in enumerate(TASKS, 1):
            ...
            sys.stdout.write(
                f"[{i:02d}/{total_tasks:02d}] [{category}] "
                f"{instruction[:50]}... "
            )
            sys.stdout.flush()
            ...
            try:
                ...
                if status == "completed":
                    ...
                    sys.stdout.write(
                        f"PASS ({duration:.1f}s, {tokens} tokens, "
                        f"{subtasks} subtasks)\n"
                    )
                elif status == "timeout":
                    report.timed_out += 1
                    ...
                    sys.stdout.write(
                        f"FAIL: {error_msg[:50]} ({duration:.1f}s)\n"
                    )
...
def print_report(report: StressTestReport) -> None:
    ...
    for cat_name, cat_stats in sorted(report.categories.items()):
        ...
        sys.stdout.write(
            f"  {cat_name:<25} {cat_stats.passed:>6} "
            f"{cat_stats.failed:>6} {cat_rate:>7.1f}%\n"
        )
    ...
    if failures:
        ...
        for f in failures:
            sys.stdout.write(
                f"  [{f.index:02d}] [{f.category}] {f.status}: "
                f"{f.instruction[:50]}...\n"
            )
            if f.error:
